 ...
 */
async function reportStatus(meetingId: string, status: MeetingStatus, extra?: Record<string, unknown>): Promise<void> {
  try {
 ...
 */
async function handleJoin(command: BotCommand): Promise<void> {
  ...
  try {
    ...
    reportStatus(meetingId, 'error', { errorMessage: (error as Error).message });
    activeBots.delete(meetingId);
  }
}
 ...
 */
async function handleStop(command: BotCommand): Promise<void> {
  const { meetingId } = command;
  const bot = activeBots.get(meetingId);
  
  if (bot) {
    console.log(`[Worker] Stopping: ${meetingId}`);
    try { await bot.stop(); } catch (e) { console.error(`[Worker] Stop error:`, e); }
    activeBots.delete(meetingId);
  } else {
    ...
    try {
      ...
      if (response.Messages && response.Messages.length > 0) {
        for (const message of response.Messages) {
          try {
            const command: BotCommand = JSON.parse(message.Body || '{}');
